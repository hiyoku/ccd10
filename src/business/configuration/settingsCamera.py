...
class SettingsCamera:
    ...
    def set_camera_settings(self, temperature_camera, pre, exp, bin, time, time_cooling, dark_photo, get_level1, get_level2):
    ...
    def get_camera_settings(self):
        return self._settings.value(c.TEMPERATURE), self._settings.value(c.PREFIXO), self._settings.value(c.EXPOSICAO),\
               ...
               self._settings.value(c.DARK_PHOTO), self._settings.value(c.GET_LEVEL1), self._settings.value(c.GET_LEVEL2)
